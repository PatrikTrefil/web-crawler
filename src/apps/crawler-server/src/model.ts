...
export default class Model implements IModel {
            ...
            if (recordResult.records.length > 0) {
                const lastExecutionId =
                    lastExecutionResult.records.length > 0
                        ? lastExecutionResult.records[0].get("lastExecution.id")
                        : null;
                const resObj = recordResult.records[0].get("record").properties;
                record = {
                    id: resObj.id,
                    url: resObj.url,
    ...
    // TODO: implement methods related to crawl executions
    /**
     * @returns true if the webpage was created; false if the webpage already exists
     */
    private async createWebPage(url: string): Promise<boolean> {
        const session = this.driver.session();
        let numberOfCreatedNodes;
        try {
            const result = await session.run(
                "MERGE (webpage:WebPage { url: $url })",
                { url: url }
            );
    ...
    public async createExecutionLink(
        ...
        // TODO: what happens if websiteRecordId is invalid
        await this.createWebPage(webPageURL);
        const session = this.driver.session();
        ...
        try {
            const result = await session.run(
                 ...
                 RETURN e.id`,
                {
                    ...
                    title: title,
                    crawlTime: neo4j.types.DateTime.fromStandardDate(crawlTime),
                }
            );
            if (result.records.length > 0) {
                createdExecutionLinkId = result.records[0].get("e.id");
            } else {
                // find error
                const recordResult = await session.run(
                    "MATCH (record:Record { id: $websiteRecordId }) RETURN record",
                    {
                        websiteRecordId: websiteRecordId,
                    }
                );
                if (recordResult.records.length === 0)
                    throw "websiteRecordId not found";
                const webPageResult = await session.run(
                    "MATCH (webpage:WebPage { url: $webPageURL }) RETURN webpage",
                    {
                        webPageURL: webPageURL,
                    }
                );
                if (webPageResult.records.length === 0)
                    throw "webPageURL not found";
                throw "Failed";
            }
        } catch (e) {
            session.close();
    ...
    public async createLinkBetweenWebPages(
        ...
        await this.createWebPage(fromWebPageURL);
        await this.createWebPage(toWebPageURL);
        const session = this.driver.session();
        let createdLinkId;
        try {
            const result = await session.run(
            ...
            if (result.records.length > 0)
                createdLinkId = result.records[0].get("e.id");
            else throw "Failed";
        } catch (e) {
            session.close();
            throw e;
    ...
    public async deleteExecution(crawlExecutionId: string): Promise<boolean> {
        ...
        try {
            ...
            const lonelyWebpageDeleteResult = await session.run(
            ...
            const deleteAllLinksResult = await session.run(
            ...
            const deleteExecutionRelationshipResult = await session.run(
